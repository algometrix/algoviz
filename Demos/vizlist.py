    ...
    def __init__(self, array, title_name='Array', sleep_time=0, highlight_color='red', show_init=True, parent=None,
        ...
        if len(array) > 0 and isinstance(array[0], list):
            ...
            for i in range(len(array)):
                self._array.append(
                    VizList(array[i], show_init=False, parent=self, title_name=f'{title_name}-'+str(i), override_get=False, column_index=self.col_index, row_index=self.row_index)
                    )
        else:
            self._array = array
    ...
    def __getitem__(self, *args, **kwargs):
        ...
        if not isinstance(res, list):
            ...
            if not isinstance(args[0], int):
                ...
                return VizList(res, show_init=False, column_index=self.col_index)
            else:
    ...
    def show_list(self, table_name='List', show_index=True, highlight=None, is_splice=False, one_dimension=True):
        ...
        if list_type == list_1d:
            ...
            if show_index:
                if self.row_index:
                    table.add_column('_')
                for i in self.col_index or range(col):
                    table.add_column(str(i))
            for i in range(row):
                if highlight:
                    if self.row_index:
                        table.add_row(self.row_index[i], *self.render_list(display[i], highlight_color=self.highlight_color,
